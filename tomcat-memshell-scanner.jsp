...
<center>
    <div>
        <%!
            ...
            public Object getApplicationContext(HttpServletRequest request) throws NoSuchFieldException, IllegalAccessException {
                Object context = request.getSession().getServletContext();
                Field _context = context.getClass().getDeclaredField("context");
                _context.setAccessible(true);
                Object appContext = _context.get(context);
            ...
            public synchronized Valve[] getValveList(HttpServletRequest request) throws Exception {
                StandardContext standardContext = (StandardContext)getStandardContext(request);
                Valve[] valveList = standardContext.getPipeline().getValves();
                return valveList;
            }
            ...
            public synchronized void deleteListener(HttpServletRequest request, String listenerName) throws Exception{
                StandardContext standardContext = (StandardContext)getStandardContext(request);
                //ApplicationContext applicationContext = (ApplicationContext)getApplicationContext(request);
                Object[] listeners = standardContext.getApplicationEventListeners();
                List<Object> listenerList = new CopyOnWriteArrayList();
                for(Object listener : listeners){
                    if(listener.getClass().getName().equals(listenerName)){
                        System.out.println("Remove Listener : "+listenerName);
                    }else{
                ...
                Field _listenersList = standardContext.getClass().getDeclaredField("applicationEventListenersList");
                _listenersList.setAccessible(true);
                _listenersList.set(standardContext,listenerList);
            }

            public synchronized void deleteValve(HttpServletRequest request, String valveName) throws Exception{
                StandardContext standardContext = (StandardContext)getStandardContext(request);
                Valve[] valves = getValveList(request);
                Pipeline pipeline = standardContext.getPipeline();
                for(Valve v : valves){
                    if(v.getClass().getName().equals(valveName)){
                        System.out.println("Remove Valve : "+valveName);
            ...
            public synchronized void deleteWebSocket(HttpServletRequest request, String wsName) throws Exception{
                ServerContainer container = (ServerContainer) request.getServletContext().getAttribute(ServerContainer.class.getName());
                Field configExactMatchMap = container.getClass().getDeclaredField("configExactMatchMap");
                configExactMatchMap.setAccessible(true);
                Map map = (Map)configExactMatchMap.get(container);
                if(map.containsKey(wsName)){
                    System.out.println("Remove websocket : "+wsName);
                    map.remove(wsName);
                }
        ...
        <%
            ...
            if (action != null && action.equals("kill") && filterName != null) {
                ...
                out.print("<a href=\"./tomcat-memshell-scanner.jsp\">filter删除操作已执行，点击跳转回扫描页</a>");
            } else if (action != null && action.equals("kill") && servletName != null) {
                deleteServlet(request, servletName);
                out.print("<a href=\"./tomcat-memshell-scanner.jsp\">servlet删除操作已执行，点击跳转回扫描页</a>");
            } else if (action != null && action.equals("kill") && listenerName != null) {
                deleteListener(request, listenerName);
                out.print("<a href=\"./tomcat-memshell-scanner.jsp\">listener删除操作已执行，点击跳转回扫描页</a>");
            } else if (action != null && action.equals("kill") && valveName != null) {
                deleteValve(request, valveName);
                out.print("<a href=\"./tomcat-memshell-scanner.jsp\">valve删除操作已执行，点击跳转回扫描页</a>");
            } else if (action != null && action.equals("kill") && wsName != null) {
                deleteWebSocket(request, wsName);
                out.print("<a href=\"./tomcat-memshell-scanner.jsp\">websocket删除操作已执行，点击跳转回扫描页</a>");
                ...
                for (Valve valve : newValves) {
                    out.write("<tr>");
                    out.write(String.format("<td style=\"text-align:center\">%d</td><td>%s</td><td>%s</td><td>%s</td><td style=\"text-align:center\"><a href=\"?action=dump&className=%s\">dump</a></td><td style=\"text-align:center\"><a href=\"?action=kill&valveName=%s\">kill</a></td>"
                            , seq + 1
                ...
                out.write("</tbody></table>");
                // todo:Scan websocket
                List<ServerEndpointConfig> endpointConfigs = getEndpointConfigs(request);
                if (endpointConfigs == null || endpointConfigs.size() == 0) {
                    ...
                    out.write("<table border=\"1\" cellspacing=\"0\" width=\"95%\" style=\"table-layout:fixed;word-break:break-all;background:#f2f2f2\">\n" +
                            ...
                            "        <th width=\"20%\">Patern</th>\n" +
                            "        <th width=\"30%\">Websocket classLoader</th>\n" +
                            "        <th width=\"35%\">Websocket class file path</th>\n" +
                            "        <th width=\"5%\">dump class</th>\n" +
                            "        <th width=\"5%\">kill</th>\n" +
                            "    </thead>\n" +
                            "    <tbody>");
                    ...
                    for (ServerEndpointConfig cfg : endpointConfigs) {
                        out.write("<tr>");
                        out.write(String.format("<td style=\"text-align:center\">%d</td><td>%s</td><td>%s</td><td>%s</td><td>%s</td><td style=\"text-align:center\"><a href=\"?action=dump&className=%s\">dump</a></td><td style=\"text-align:center\"><a href=\"?action=kill&wsName=%s\">kill</a></td>"
                                , endpointIndex + 1
